...
func (o *operator) GetNodeResources(instance string, startTime time.Time, endTime time.Time, step time.Duration) ([]prometheus.Metric, error) {
	exprMap := func(string) map[string]string {
		...
		exprMap["node_network_drop_rate"] = fmt.Sprintf("topk(1, 1-(increase(cpds_node_ping_recv_count_total{instance=~\"%s.*\"}[1m])/increase(cpds_node_ping_send_count_total{instance=~\"%s.*\"}[1m])<=1 or vector(1)))",instance,instance)
		...
		exprMap["node_network_recive_error_rate"] = fmt.Sprintf("scalar(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))/(scalar(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_packets_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))+scalar(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface)))",instance,instance,instance)
		exprMap["node_network_transmit_error_rate"] = fmt.Sprintf("scalar(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))/(scalar(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_packets_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))+scalar(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface)))",instance,instance,instance)
		...
		exprMap["node_retransm_rate"]=fmt.Sprintf("sum(increase(cpds_node_netstat_tcp_retrans_segs{instance=~\"%s.*\"}[1m])) / sum(increase(cpds_node_netstat_tcp_out_segs{instance=~\"%s.*\"}[1m]))",instance,instance)
...
func (o *operator) GetClusterResource(startTime time.Time, endTime time.Time, step time.Duration) ([]prometheus.Metric, error) {
	exprMap := func() map[string]string {
		...
		exprMap["cluster_network_drop_rate"] = "topk(1, (1-sum(increase(cpds_node_ping_recv_count_total[1m]))/sum(increase(cpds_node_ping_send_count_total[1m])))>=0 or vector(0))"
		exprMap["cluster_network_recive_error_rate"] = "sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_errors_total{interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))/(sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_packets_total{interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))+sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_errors_total{interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface)))"
		exprMap["cluster_network_transmit_error_rate"] = "sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_errors_total{interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))/(sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_packets_total{interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))+sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_errors_total{interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface)))"
