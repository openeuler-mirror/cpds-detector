...
func (o *operator) GetNodeResources(instance string, startTime time.Time, endTime time.Time, step time.Duration) ([]prometheus.Metric, error) {
	exprMap := func(string) map[string]string {
		...
		exprMap["node_network_recive_error_rate"] = fmt.Sprintf("sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))/(sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_packets_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))+sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_receive_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface)))",instance,instance,instance)
		exprMap["node_network_transmit_error_rate"] = fmt.Sprintf("sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))/(sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_packets_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface))+sum(cpds_node_network_info{mask=~\".+\"}*0+on(interface,instance) group_right sum(increase(cpds_node_network_transmit_errors_total{instance=~\"%s.*\",interface!~\"lo|bond[0-9]|cbr[0-9]|veth.*|vir.*|docker.*|vnet.*|br.*|tap.*|tunl.*\"}[1m])) by (instance,interface)))",instance,instance,instance)
